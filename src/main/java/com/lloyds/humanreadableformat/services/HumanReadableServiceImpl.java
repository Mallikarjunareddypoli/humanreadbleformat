...
import static com.lloyds.humanreadableformat.constants.HumanReadableConstants.*;

@Service
public class HumanReadableServiceImpl implements HumanReadableService {
    ...
    public HumanReadable timeHumanReadableFormat(){
        ...
        return new HumanReadable(timeHumanReadableFormat(hours, minutes));
    }
     ...
     * @return
     */
    public HumanReadable timeHumanReadableFormat(final String time) throws InputTimeFormatException {
        String inWords=null;
        if(isValidTime(time)) {
           ...
           throw new InputTimeFormatException(TIME_FORMAT_ERROR);
        }
        return new HumanReadable(inWords);
    }
     ...
     * @return
     */
    private  String timeHumanReadableFormat(final int hours, final int minutes) {
        String inWords=null;
        if (minutes == 0)
     ...
     * @return
     */
    private  String getMinutesInWords(final int _minutes) {
        String inWords=null;
        ...
        if(a==1)
            ...
            inWords = TENS_MULTIPLE[a] + HYPHEN + SINGLE_DIGITS[b].toLowerCase();
        else
